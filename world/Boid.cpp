...
const float DT = 0.01f; //TODO(remove)
...
void Boid::setY(float y) {
    this->position.y = y;
    this->box.top = y - this->box.height * 0.5f;
}


quadtree::Box <float> Boid::getBox() const {
    return this->box;
}

template <typename GetBoxFunc>
std::vector<Boid> Boid::getNeighbors(float radius, const quadtree::Quadtree<Boid*, GetBoxFunc>& quadtree) const {

    float x = this->getX();
    float y = this->getY();

    quadtree::Box qbox = quadtree::Box<float>(x - radius,
                                                y - radius,
                                                2.0 * radius,
                                                2.0 * radius);
...
template <typename GetBoxFunc>
void Boid::update(const quadtree::Quadtree<Boid*, GetBoxFunc>& quadtree) {
    float dt = 0.01;
...
std::optional<std::pair<sf::Vector2f, sf::Vector2f>> Boid::getBoidsAvgInfo(const std::vector<Boid>& boids) {
